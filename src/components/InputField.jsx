...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    ...
    const hoverFocusClasses = isFocused
      ? "shadow-[0_4px_12px_rgba(30,58,138,0.4)] border-[#1e3a8a]"
      : "border-white/20 hover:border-[#1e3a8a]";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 text-gray-400 transition-colors group-hover:text-[#1e3a8a]">
        ...
        <label
          ...
          className={`
            ...
            peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-[#1e3a8a] peer-focus:text-sm
          `}
        >
          {label}
        </label>
      ...
      <div
        className={`
          ...
          ${isFocused ? "border-[#1e3a8a]" : "border-transparent"}
        `}
      ></div>
    </div>
  );
